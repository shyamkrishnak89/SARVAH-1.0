...
public function sendQuery($name, $email, $subject, $message){
	$to = "kizhuthullytemple@gmail.com";
			
			$msg = '<p>Hello,</p>
			<p>Myself '.$name.'('.$email.')</p>
			<p>'.$message.'</p>
			';
			
			echo smtp_mailer($to,$subject, $msg);
			$action = "Query from-".$_SESSION['user']."Mail Id:".$email;
	...
	public function memberPasswordChange($username, $newPassword){
		$sqlInsertQuery = "
				UPDATE ".$this->memberTable." 
				SET password = '".$newPassword."' 
				WHERE housenumber = '".$username."'";	
        
		mysqli_query($this->dbConnect, $sqlInsertQuery);
		$action = "Password Change-UN:". $username;
			$this->activity_log($username,$action);
	}	
	...
	public function updateInvoice($POST) {
		...
		if($POST['invoiceId']) {	
			$sqlInsertInvoice = "
				...
				SET order_total_after_tax = '".$POST['totalAftertax']."', order_amount_paid = '".$amount_paid."', order_total_amount_due = '".$POST['amountDue']."', note = '".$POST['floatingNotes']."' 
